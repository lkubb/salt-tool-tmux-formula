...
We will see a quick setup to configure the ``tool_tmux`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_tmux`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_tmux`` formulas under ``/srv/salt/tool_tmux/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_tmux/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_tmux/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_tmux/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_tmux/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_tmux`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_tmux`` formulas under ``/srv/salt/tool_tmux/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_tmux/parameters/roles
...
We create a configuration for the role ``tool_tmux/server`` in ``/srv/salt/tool_tmux/parameters/roles/tool_tmux/server.yaml``:
...
We create another configuration for the role ``tool_tmux/client`` in ``/srv/salt/tool_tmux/parameters/roles/tool_tmux/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_tmux/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_tmux/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_tmux/parameters/dns:domain/  '/srv/salt/tool_tmux/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-tmux-as-json:
      test.nop:
        - name: {{ tmux | json }}
...
.. code-block:: sls
    ...
    tmux-config-file-file-managed:
      file.managed:
        - name: {{ tmux.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='tmux-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ tmux.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``tmux`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ tmux['winner'] }}
